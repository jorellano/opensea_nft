...
st.title(f"OpenSea NFT API Explorer - {endpoint}")
...
if endpoint == 'Event':
    ...
    for event in events['asset_events']:
        if event_type == 'offered_entered':
            ...
            event_list.append([event]['created_date'], bidder, float)['asset']['collection']['name'], event['asset']['token_id']])
...
if endpoints == 'Assets':
    ...
    owner = st.sidebar.text_input('0wner')
    collection = st.sidebar.text_input('Collection')
    params = {'owner': owner}
    if collection:
